...
import { FlatList, Dimensions, View, Text, Image, StyleSheet, TouchableOpacity } from 'react-native';
...
const Programmation = ({ artistes }) => {
    ...
    useEffect(() => {
        const leFiltreScenes = filtres(artistes, listeScenesArray);
        const leFiltreStyles = filtres(leFiltreScenes, listeStylesMusicauxArray);
        const filtrerParJour = filtreJour(leFiltreStyles, jour);
        const trierParJour = trieJour(filtrerParJour);
        const rechercherParNomArtistes = rechercheNomArtiste(trierParJour, rechercher);
        setFilterArtistes(rechercherParNomArtistes);
    ...
    function stylesMusicauxArray(artistes) {
        ...
        artistes.forEach(artiste => {
            ...
            if (styleMusicalRequete.length > 0) {
                ...
                const styleMusicalTrie = styleMusicalRequete.split(",").map(s => s.trim());
    ...
    return (
        <View style={{
            ...
            <View style={styles.topBar
                ...
                <TouchableOpacity onPress={() => { setRecherche(!recherche) }}>
                    <Image source={require('../icones/rechercher.png')} style={styles.icone} />
                </TouchableOpacity>
                <TouchableOpacity style={styles.text} onPress={() => { setJour('samedi') }}>
                    <Text>SAMEDI</Text><Text style={jour === 'samedi' ? styles.selectText : styles.select}>10 Juin</Text>
                </TouchableOpacity>
                <TouchableOpacity style={styles.text} onPress={() => { setJour('dimanche') }}>
                    <Text>DIMANCHE</Text><Text style={jour === 'dimanche' ? styles.selectText : styles.select}>11 Juin</Text>
                </TouchableOpacity>
                <TouchableOpacity onPress={() => { setFiltre(!filtre) }}>
                    {filtre ?
                        <Image source={require('../icones/croix.png')} style={styles.icone} /> :
                            ...
                            renderItem={({ item }) =>
                                <View>
                                    <CheckBox
                                        ...
                                        onPress={() => { inversionLogiqueStylesMusicaux(item.id, listeStylesMusicauxArray) }}
                            ...
                            renderItem={({ item }) =>
                                <View>
                                    <CheckBox
                                        title={item.scene}
                                        checked={item.selected}
                                        onPress={() => { inversionLogiqueStylesMusicaux(item.id, listeScenesArray) }}
                                        style={styles.checkbox}
                                        checkedIcon='music'
                                        checkedColor='#6DBD38'
                                        uncheckedIcon='music'
